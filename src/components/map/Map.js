    ...
    <div className="map-container">
      ...
      <div className="map-layout">
        <div className="map-wrapper">
          ...
          >
            <MapEventHandler onBoundsChange={handleBoundsChange} isZoomingRef={isZoomingRef} mapRef={mapRef} />
            <TileLayer
              attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
              url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
            />
            {markers.map((marker, index) => (
              <Marker key={marker.resource.id || index} position={marker.position}>
                <Popup>
                  <div className="popup-content">
                    <h3>{marker.resource.name}</h3>
                    <p><strong>Category:</strong> {marker.resource.category}</p>
                    <p><strong>Address:</strong> {marker.resource.address}</p>
                    {marker.resource.phone && (
                      <p><strong>Phone:</strong> <a href={`tel:${marker.resource.phone}`}>{marker.resource.phone}</a></p>
                    )}
                    {marker.resource.website && (
                      <p><strong>Website:</strong> <a href={marker.resource.website} target="_blank" rel="noopener noreferrer">Visit Website</a></p>
                    )}
                    {marker.resource.description && (
                      <p className="description">{marker.resource.description}</p>
                    )}
                  </div>
                </Popup>
              </Marker>
        ...
        <div className="resources-sidebar">
          <div className="sidebar-header">
            ...
            <span className="resource-count">{visibleResources.length} visible</span>
          </div>
          <div className="resources-list">
            {visibleResources.length === 0 ? (
              <div className="no-resources">
                <p>No resources in current view. Pan or zoom the map to see resources.</p>
              </div>
            ) : (
